...
import TvShowLink from "../../components/TvLink"; // TvShowLink로 변경
import "./Home.css";
...
const Home = () => {
  ...
  const getTvShows = async () => {
    ...
    const {
      ...
      "https://api.themoviedb.org/3/discover/tv?&page=1&language=ko&region=kr&sort_by=vote_count.desc&api_key=250604987b9bcb91e2f812b87db35ebf"
    );

    const tvShowsWithGenres = results.map((show) => ({
      // 각 TV 쇼 객체에 장르 이름을 추가하여 새로운 배열 tvShowsWithGenres를 만들었음
      ...show,
    ...
    <section className="container">
      {isLoading ? ( // isLoading 상태에 따라 로딩 중이면 로딩 메시지를, 그렇지 않으면 TV 쇼 목록을 표시
        ...
        <div className="tv-shows"> {/* 클래스 이름을 movies에서 tv-shows로 변경 */}
          {tvShows.map((show) => ( // tvShows 배열을 반복하여 각 TV 쇼에 대한 TvShowLink 컴포넌트를 생성
            <TvShowLink
              key={show.id}
              id={show.id}
