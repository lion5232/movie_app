...
const Home = () => {
  ...
  const [tvShows, setTvShows] = useState([]); // tvShows 상태를 초기값으로 빈 배열을 설정하여 TV 쇼 목록을 저장
  ...
  const getTvShows = async () => {
    ...
    const {
      ...
      "https://api.themoviedb.org/3/tv/top_rated?language=ko&api_key=250604987b9bcb91e2f812b87db35ebf"
    );
        ...
        <div className="tv-shows">
          {tvShows.map((show) => ( // tvShows 배열을 반복하여 각 TV 쇼에 대한 TvLink 컴포넌트를 생성
            <TvLink
              ...
              summary={show.overview.slice(0, 100)} // 개요를 100자로 잘라서 표시
              poster={`https://image.tmdb.org/t/p/w500${show.poster_path}`} // 포스터 URL
              first_air_date={show.first_air_date} // 첫 방영일
              rating={show.vote_average + "점"} // 평점
              genres={show.genres} // 장르
            />
          ))}
        </div>
      )}
